...
| Phase | Scope | Status |
|-------|-------|--------|
| **Phase 1 — Understand + Frontend** | Odoo XML-RPC tools (connect, introspect → System Map), addon source scan, understanding report, web UI (chat + dashboard + PDF report download). The deterministic, no-LLM foundation. | ✅ **Done — runnable** |
...
src/sentinel/
  ...
  web/
    ...
    static/index.html        single-page UI (chat + System Map dashboard + PDF report download)
...
Open `http://127.0.0.1:8800`, fill in the connection fields (Odoo URL, database, credentials,
module name, and optionally the addon source path), then click **Understand** — the System Map
fills in from the live instance (deterministic, no LLM). Then chat with it, or run a one-shot audit.

Connection field defaults can be set via environment variables so you don't have to re-enter them:
```powershell
...
$env:SENTINEL_ADDONS_PATH = "C:\path\to\my_addon"
```
